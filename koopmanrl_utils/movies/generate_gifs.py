...
class ArgumentParser(Tap):
    seed: int = 123  # seed of the experiment (default: 123)
    data_folder: bool = True  # Folder containing trajectory data
    save_every_n_steps: int  # Save a frame every n steps
    plot_uncontrolled: bool = False  # Plot the uncontrolled system dynamics
...
def main():
    ...
    ma_window_size = default_ma_windows[metadata["env_id"]] if args.ma_window_size is None else args.ma_window_size
    ...
    for trajectory_num in range(main_policy_trajectories.shape[0]):
        ...
        for step_num in range(main_policy_trajectories.shape[1]):
            if step_num == 0 or (step_num + 1) % args.save_every_n_steps == 0:  # Only save every n steps
                ...
                # Place Green dot at the reference point
                ...
                # Set axis limits according to the trajectory limits, using zero_policy
                # only if plot_uncontrolled is True
                if args.plot_uncontrolled:
                    # Maximums
        ...
        for cost_num in range(main_policy_costs.shape[0]):
            ...
            moving_average_log_all_cost_ratios = moving_average(log_all_cost_ratios, n=ma_window_size, keep_first=False)
            ...
            moving_average_log_all_cost_ratio_x_values = np.arange(
                cost_x_values_start_index, moving_average_log_all_cost_ratios.shape[0] + cost_x_values_start_index
            )
                ...
                if step_num == 0 or (step_num + 1) % args.save_every_n_steps == 0:
                    ...
                    if step_num >= cost_x_values_start_index:
                        moving_average_log_cost_ratio = moving_average_log_all_cost_ratios[
                            : (step_num + 1) - cost_x_values_start_index
                        ]
