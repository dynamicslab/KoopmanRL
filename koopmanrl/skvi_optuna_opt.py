...
class ArgumentParser(Tap):
    ...
    storage_dir: str = "/home/lpaehler/Work/ReinforcementLearning/KoopmanRLLaptop/KoopmanRL/BayesianOptimization/koopman-rl/tuning"  # Directory into which to store the result  # noqa: E501
    output_file: str = "skvi_linear_system_params"  # Name of the json with the best configuration


def evaluate(config):
    # If the batch size does not accomodate the combination, round down to the next power of 2
    if config["num-paths"] * config["num-steps-per-path"] < 2**14:
        batch_size_override = pow(
            2,
            ceil(log(config["num-paths"] * config["num-steps-per-path"]) / log(2)) - 1,
        )
        print("Batch size overwritten with: ", batch_size_override)
    else:
        # If condition is not triggered, retain the normal batch scaling
        batch_size_override = 2**14
    ...
    _experiment = skvi_tuning_wrapper(
        ...
        number_of_training_epochs=config["number-of-train-epochs"],
...
def main():
    ...
    # Definition of the search space --> Needs to be proofread and dialed in
    search_space = {
        "env-id": args.env_id,
    ...
    # Limit each trial itself to a set of CPU cores
